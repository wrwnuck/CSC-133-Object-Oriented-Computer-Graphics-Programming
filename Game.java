...
public class Game extends Form implements Runnable{
	...
	private BGSound bgSound = new BGSound("hqBackground.mp3");
	public Game() {
		gw = new GameWorld();
		...
		toolbar.setTitle("Sili-Challenge Game");
	...
	public void playSounds() {
		...
		if(gw.isEnergyCol()) {
			...
			Sound energySound = new Sound("boing.wav", "audio/wav");
			if(!gw.isIspause())
				energySound.play();
		...
		if(gw.isCrashCol()) {
			...
			Sound crashSound = new Sound("CANTDO2.wav", "audio/wav");
			if(!gw.isIspause())
				crashSound.play();
		}
		...
		if(gw.isDeathCol()) {
			gw.setCrashCol(false);
			Sound deathSound = new Sound("Marine_Death01.mp3", "audio/wav");
			if(!gw.isIspause())
				deathSound.play();
		}
		if(gw.isIspause()) {
