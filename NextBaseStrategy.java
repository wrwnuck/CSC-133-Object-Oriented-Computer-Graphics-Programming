package com.mycompany.a3;

import com.codename1.util.MathUtil;
public class NextBaseStrategy implements IStrategy {

	private NonPlayerCyborg npc;
    private GameWorld tgw;

    public NextBaseStrategy(NonPlayerCyborg thisCyborg, GameWorld gw) {
        npc = thisCyborg;
        tgw = gw;
    }

    @Override
    public void setStrategy() {
        // TODO Auto-generated method stub
        int nextBase = npc.getLastBase() + 1;
        double pX = 0, pY = 0;
        IIterator theElements = tgw.gameObjectList.getIterator();

        Object currentObj;
        while( theElements.hasNext() ){
            currentObj = theElements.getNext();
            if(currentObj instanceof Base)
                if( ((Base)currentObj).getSequenceNumber() == nextBase ){
                    // found the next base they need to get to, save coordinates
                    pX = ((Base)currentObj).getX();
                    pY = ((Base)currentObj).getY();
                }
        }

        double dx, dy;

        dx = pX - npc.getX();
        dy = pY - npc.getY();

        if( dy == 0 )
            if( dx < 0 ){
                // target base is directly to the left
                npc.setHeading(270);
                return;
            }
            else if( dx > 0 ){
                // target base is directly to the right
                npc.setHeading(90);
                return;
            }
// is x val the same?
        if( dx == 0 )
            if( dy > 0 ){
            
                npc.setHeading(0);
                return;
            }else if( dy < 0 ){
                
                npc.setHeading(180);
                return;
            }

        // target wasn't at a 90 degree interval,
        // need to determine the angle
        
        int angle = (int)(Math.toDegrees(MathUtil.atan(dx/dy)));

        // got the angle, now figure out which quadrant to put it in
        if( dx > 0 ){ 
            if( dy > 0 ){
                // top right
                npc.setHeading( Math.abs(angle) );
                return;
            }else if( dy < 0){
                // bottom right
                npc.setHeading( 180 - angle );
                return;
            }
        }else if( dx < 0 ){
            if( dy > 0){
                // top left
                npc.setHeading( 360 - angle );
                return;
            }else if( dy < 0){
                // bottom left
                npc.setHeading( 180 + Math.abs(angle) );
                return;
            }
        }

     }

    @Override
    public void invokeStrategy() {
        setStrategy();

    }
}